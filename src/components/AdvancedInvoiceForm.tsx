...
export const AdvancedInvoiceForm = ({ onClose, invoice }: AdvancedInvoiceFormProps) => {
  ...
  const getPreviewData = () => ({
    ...
    lineItems: [
      { id: '1', description: 'Construction Services', quantity: 1, rate: calculateSubtotal(), amount: calculateSubtotal() }
    ],
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    toast({
      ...
      description: `Advanced invoice ${formData.invoiceNumber} has been created with ${formData.hasPaymentPlan ? 'payment plan' : 'standard terms'} using ${selectedTemplate} template.`,
    });
