...
// import { DragDropContext, Droppable, Draggable } from 'react-beautiful-dnd'; // Temporarily disabled
import {
  Calendar,
  Users,
  FileText,
...
export const EditableQuickActions = () => {
  ...
  const getThemeGradients = () => {
    if (theme === "dark") {
      return {
        clientIntake:
          "from-blue-600 to-purple-700 hover:from-blue-700 hover:to-purple-800",
        general:
  ...
  const handleDragEnd = (result: any) => {
    ...
    console.log("Drag and drop would reorder actions");
  };
      ...
      <Card className={`border shadow-lg ${themeGradients.cardBg}`}>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {clientIntakeActions.map((action, index) =>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            {generalActions.map((action, index) =>
              renderActionButton(action, index + clientIntakeActions.length),
            )}
          </div>
        </CardContent>
      </Card>
